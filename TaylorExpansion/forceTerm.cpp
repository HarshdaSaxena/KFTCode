...
a_min (0.001), a_max (1.0), k_min (0.1), k_max (10.0), k_scale (0.0), k_star (0.1),
a_save (-1.0), k_save (-1.0), f_k_save (-1.0), f_r_save (-1.0), n_table (256),
...
double forceTerm::taylorfactor (double k, double a) /*3*int g_H *G_eff* 1/(a^2 D_plus' E)* D_plus^2*sigma_J^2 D_plus' gets cancelled from t->a variable change in intg*/
{ screeningParameters q (cosmological_model,a);
  astro::integrator integrate
    ([&] (double x)
    { //astro::cosmologicalParameters * p = cos_model->getParameters ();
      //screeningParameters * q = reinterpret_cast<screeningParameters*> (p->par);
      return
        3.0*q.param_B*q.param_b*pow(k_star/k,q.param_a)*1./a_min*propagator_h->g_qp(a,x)*
        pow(Dplus(x,k),2.)*(sigma_J_prime_sq_table(k,x))/pow(x/a_min,2.)/E(x); },1.0e-2);
  /*std::cout<<integrate (a_min, a);*/
...
{ return (1.0 + taylorfactor(k,a))*Dplus(a,k)*Dplus(a,k)*amplitude*linear_spectrum->operator () (k, a_norm)*
  exp (interaction_term_normal (k, a)); }
