...
{ 
  astro::integrator integrate_J_prime ([&] (double x)
      { return
          (1-y*x)/(1+y0*y0 + y*y -2*x*y)/pow(1+y*y-2*x*y,1.5) *(pow(1+ pow(1+y*y-2*y*x,1.5)/pow(y_star,3),0.5)-1); },1.0e-2);
...
/*
  ...
  switch(proptype){
    case (0):{
      ...
      break;}
    case(1):{
      return interaction_term_zeldovich (k, a);
      break;}
    default:{
...
/*switch(proptype){
      ...
      break;}
      case(1):{
      data.y[i] = 1.0-exp (-0.5*interaction_term_zeldovich (data.x[i], a));
      break;}
      default:{
        throw std::runtime_error ("Error: Unknown propagator type");
        break;
...
{ return cosmic_structures->Dplus(a)/D_plus_min;}
...
double forceTerm::m(double a, double k)
{ return propagator->g(a);}

double forceTerm::m_dot(double a, double k)
